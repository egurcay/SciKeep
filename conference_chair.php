...
from (submissionReferenced natural join (submission natural join (has natural join confChair))), publish
where  sub_ID_publish = sub_ID AND conf_chair_ID = hID AND hsub_ID = sub_ID AND referenced = title and conf_chair_ID = '$uname'");
$citation_count = $query->fetch_array(MYSQLI_BOTH);



$query = mysqli_query($conn, "SELECT count(*) as submitted_papers
from (submission join (has join confChair))
where conf_chair_ID = '$uname' and sub_ID = hsub_ID and hID = conf_chair_ID");
$submitted_paper_count = $query->fetch_array(MYSQLI_BOTH);

$query = mysqli_query($conn, "SELECT count(*) as published_papers
from (publish join (submission join (has join confChair)))
where conf_chair_ID = '$uname' and sub_ID = hsub_ID and hID = conf_chair_ID and sub_ID_publish = sub_ID");
...
$papers = mysqli_query($conn, "SELECT distinct title, publish_date, name, count(referenced) as reference_count
        from (institution join (organises join (pubMech join (submitted_to join (publish join submissionReferenced))))), (submission join (has join confChair))
        where conf_chair_ID = '$uname' and conf_chair_ID = hID and hsub_ID = sub_ID and sub_ID = submit_ID and sub_ID_publish = sub_ID and submit_ISBN = ISBN and org_ISBN = ISBN and org_name =  name and referenced = title
        group by title, publish_date, name;");

$my_conferences = mysqli_query($conn, "select sessions_ID , room, conf_name, location 
from (confChair join (manages_ses join (sessions join (conf_present join conference))))
where conf_chair_ID = man_ID and man_ses_ID = sessions_ID and sessions_ID = conf_session_id and conf_ISBN_present = conf_ISBN and conf_chair_ID = '$uname'; ");
                    ...
                    <form role="search" method="get" class="header__search-form" action="searchresults.php">
                        <label>
                            <span class="hide-content">Search Paper</span>
                             <label>
                                <form action="">
                            <input type="radio" name="by" value="Title" checked="on"> Title
                            <input type="radio" name="by" value="Author"> Author
                            <input type="radio" name="by" value="Topic"> Topic
                             </form>
                        </label>
                ...
                <nav class="header__nav-wrap">
                    ...
                    <ul class="header__nav">
                        ...
                        <li class="has-children">
                            ...
                            <ul class="sub-menu">
                            <li><a href="category.html">Embedded Sysytems</a></li>
                            <li><a href="category.html">Computer Architecture</a></li>
                            <li><a href="category.html">Computational Biology</a></li>
                            <li><a href="category.html">Management Information Systems</a></li>
                            <li><a href="category.html">Computers and Society</a></li>
                            <li><a href="category.html">Browse All Topics</a></li>
                            </ul>
                        </li>

                        <li><a href="authors.php" title="">Authors</a></li>
                        <li class="current"><a href="profile.php" title="">My Profile</a></li>
        ...
        <div class="row add-bottom">
            ...
            <div class="table-responsive">
                ...
                <table>
                        ...
                        while($row = mysqli_fetch_assoc($my_conferences)){
                        echo "<tr>";
                        echo " <td><a href=\"#\"> {$row["sessions_ID"]} </a></td>";
                        echo " <td>  {$row["room"]} </td>";
