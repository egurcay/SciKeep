...
$query = mysqli_query($conn, "SELECT website from  member where ID = '$uname'");
...
$papers = mysqli_query($conn, "SELECT distinct title, publish_date, name
        ...
        where author_ID = '$uname' and author_ID = hID and hsub_ID = sub_ID and sub_ID_publish = sub_ID and org_ISBN = ISBN and org_name =  name");

$awaiting = mysqli_query($conn, "SELECT distinct title, firstname, lastname, sub_ID
from (reviewerAssigned join (reviewer join member)), (author join (has join submission))
where reviewer_ID_assigned = reviewer_ID and reviewer_ID = ID and assigned_paper_ID = sub_ID and sub_ID = hsub_ID and hID = author_ID and author_ID =  '$uname';");
...
order by publish_date asc;");
    ...
    order by reference_count asc;");
        ...
        default:
            $papers = mysqli_query($conn, "SELECT distinct title, publish_date, name
        from (institution join (organises join (pubMech join (submitted_to join (publish))))), (submission join (has join author))
        where author_ID = '$uname' and author_ID = hID and hsub_ID = sub_ID and sub_ID_publish = sub_ID and org_ISBN = ISBN and org_name =  name and referenced = title
...
<head>
    ...
    ================================================== -->
    <meta charset="utf-8">
    <title>ScienceKeep - My Profile</title>
        ...
        <div class="row add-bottom">
                ...
                <div class="table-responsive">
                    ...
                    <table>
                            ...
                            while($row = mysqli_fetch_assoc($awaiting)){
                        ...
                        echo " <td> <input type=\"submit\" value = \"delete paper\" name = \"assign\" class=\"btn--primary btn--medium \"/> </td>" ;

                        if(isset($_POST['assign'])){
                          $paper_ID = $_POST['paper_ID'];
                          $title = mysqli_query($conn, "DELETE from submission where sub_ID = '$paper_ID';");
